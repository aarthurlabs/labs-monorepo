...
export function PostEditorForm({ post }: PostEditorFormProps) {
  ...
  return (
    <main className="mx-auto w-full max-w-5xl px-space-4 py-space-8 sm:px-space-6">
      <form id="post-editor-form" noValidate onSubmit={handleSubmit(onSubmit)}>
          ...
          <div className="mt-space-4 flex flex-col gap-space-3">
            ...
            <div className="grid gap-space-4 md:grid-cols-3">
              <div className="flex min-w-0 flex-col gap-[5px]">
                ...
                />
                {errors.type && (
                  <span id="type-error" className="text-label text-danger">
                    {errors.type.message}
                  </span>
                )}
              </div>

              <Field label="Número" optional error={errors.number?.message}>
                <Input
                  type="number"
                  min={1}
                  step={1}
            ...
            <div className="grid gap-space-4 md:grid-cols-2">
              <div className="flex min-w-0 flex-col gap-[5px]">
                ...
                />
                {errors.status && (
                  <span id="status-error" className="text-label text-danger">
                ...
                <Controller
                  ...
                  render={({ field }) => (
                    <Switch
                      ...
                      onBlur={field.onBlur}
                      aria-invalid={Boolean(errors.featured)}
                      aria-describedby={
                        errors.featured ? "featured-error" : undefined
                      }
                    />
                  )}
                />
                {errors.featured && (
                  <span id="featured-error" className="text-label text-danger">
            ...
            <div className="flex min-w-0 flex-col gap-[5px]">
              ...
              />
              {errors.tags && (
                <span id="tags-error" className="text-label text-danger">
                  {errors.tags.message}
                </span>
              )}
            </div>
            ...
            <div className="grid gap-space-4 md:grid-cols-2">
              <Field label="Repositório" error={errors.repositoryUrl?.message}>
                <Input
                  type="url"
                  mono
              ...
              <Field label="Projeto" error={errors.liveUrl?.message}>
                <Input
                  type="url"
                  mono
                  placeholder="https://..."
        ...
        >
          ...
          <div className="mt-space-4 flex flex-col gap-space-3">
            <Field label="Título" error={errors.learnedTitle?.message}>
              <Input
                placeholder="A ideia principal, numa frase."
                {...register("learnedTitle")}
              />
            </Field>
            <Field label="Conteúdo" error={errors.learnedContent?.message}>
              <Textarea
